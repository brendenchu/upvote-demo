...
export default function Upvote({ selected, handler }: UpvoteProps) {

  const [currentStyle, setCurrentStyle]: [string, (style: string) => void]
  = useState<string>(styles.default)
  ...
  useEffect((): void => {
    if (selected)
      setCurrentStyle(styles.selected)
    else
  ...
  return (
    <div onClick={handleClick}>
      <svg className={currentStyle} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
